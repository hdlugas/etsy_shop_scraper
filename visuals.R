...
df$avg_rating = factor(df$avg_rating, levels = c('0','4','5'), labels = c('<4', '4-4.5', '5'))
...
ggp1 = ggplot(df, aes(x=factor(accepts_custom_requests,levels=c(0,1),labels=c('No','Yes')), fill = avg_rating)) +
  ...
  theme(plot.title = element_text(hjust=0.5, size=20), panel.background=element_blank(), 
        panel.border = element_rect(color="black", fill=NA, linewidth=0.3), text = element_text(size=15))  +
  # scale_x_continuous(limits = c(0,400)) +
  xlab('Accepts Custom Requests') + ylab('Count') + labs(fill = 'Average Rating')

ggp2 = ggplot(df, aes(x=number_items_listed, fill = avg_rating)) + geom_histogram(binwidth=10) + 
...
ggp3 = ggplot(df, aes(x=avg_price, fill = avg_rating)) + geom_histogram(binwidth=100) + 
...
ggp4 = ggplot(df, aes(x=number_feature_items, fill = avg_rating)) + 
  geom_histogram(stat = 'count') + 
...
ggarrange(ggp1, ggp2, ggp3, ggp4, common.legend=TRUE, legend='right', nrow=2, ncol=2)
